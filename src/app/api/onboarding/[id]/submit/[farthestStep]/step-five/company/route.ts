...
import { employerOnboardingStepFiveSchema } from "@/schema/onboarding";
...
export async function POST(
  ...
  const { data, success, error } = employerOnboardingStepFiveSchema.safeParse(body);
  if (!success) {
    return NextResponse.json({ error: formatZodError(error) }, { status: 422 });
  }

  try {
    await CompanyOnboardingService.submitStepFive(String(params.id), params.farthestStep, data);
    return NextResponse.json({ status: 204 });
  } catch (err) {
    console.error("Error submitting company onboarding step five:", err);
