...
export async function POST(
  ...
  try {
    ...
    await connection.query(`UPDATE account SET status_id = ? WHERE company_id = ?`, [1, Number(params.id)]);
    await connection.query(`DELETE FROM onboarding WHERE company_id = ?`, [Number(params.id)]);
