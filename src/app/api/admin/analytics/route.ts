...
export async function GET(request: NextRequest) {
  try {
    ...
    const applicantCount = Array.isArray(applicantResult) ? applicantResult[0]?.count || 0 : 0;
    ...
    const companyCount = Array.isArray(companyResult) ? companyResult[0]?.count || 0 : 0;

    // Get job post count
    const [jobPostResult] = await db.query(
      `SELECT COUNT(*) as count 
       FROM job_posts 
       WHERE ${dateFilter}`
    );
    const jobPostCount = Array.isArray(jobPostResult) ? jobPostResult[0]?.count || 0 : 0;

    // Get courses distribution from ojt_profile
    const [coursesResult] = await db.query(
      `SELECT op.course, COUNT(*) as count 
       FROM ojt_profile op
       INNER JOIN account a ON op.user_id = a.user_id
       WHERE a.role_id = 3 ${timeFrame !== "all" ? `AND ${dateFilter.replace("created_at", "a.created_at")}` : ""}
       GROUP BY op.course
       ORDER BY count DESC`
    );

    const coursesData = Array.isArray(coursesResult)
      ? coursesResult.map((row: any, index: number) => ({
    ...
    const [companiesResult] = await db.query(
      `SELECT industry 
       ...
       WHERE a.role_id = 4 ${timeFrame !== "all" ? `AND ${dateFilter.replace("created_at", "a.created_at")}` : ""}`
    );
    ...
    if (Array.isArray(companiesResult)) {
      companiesResult.forEach((row: any) => {
        if (row.industry) {
          const industries = row.industry.split(",").map((ind: string) => ind.trim());
